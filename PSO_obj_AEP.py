...
for r in range(number_of_runs):
    ...
    site = IEA37Site(num_turbines,shear=PowerShear(h_ref=80, alpha=.1))
    site.default_wd = wd
    ...
    for iteration in range(max_iterations):
        ...
        for i in range(pop_size):
            ...
            if total_capacity[i, iteration] > max_capacity:
                ...
                penalty[i, iteration] = np.log(excess_capacity+1) * penalty_factor
            ...
            if total_capacity[i, iteration] < min_capacity:
                ...
                penalty[i, iteration] = np.log(remaining_capacity+1) * penalty_factor
...
#%% ERROR TRIAL            
            try:
                particle_aep[i] = windFarmModel(wt_x_loop, wt_y_loop, h=hub_heights_loop, type=type_vector_loop).aep().sum().values - penalty[i,iteration]
                ...
                # particle_aep[i] = particle_aep[i] - penalty
            ...
            if particle_aep[i] > particle_best_aep[iteration]:
            ...
            if particle_aep[i] > global_best_aep:
                # print(num_turbines)
        ...
        # print('Particle best AEP:', max(particle_aep_hist[:, iteration]))
...
#%%CALCULATE MIN AND MAX VALUES AFTER ALL RUNS        
    if r == 0:
        ...
        aep_max_iter = r
        aep_min_iter = r
    if aep_plot[-1] > aep_max:
        aep_max = lcoe_plot[-1]
        aep_max_iter = r
    if aep_plot[-1] < aep_min:
        aep_min = lcoe_plot[-1]
        aep_min_iter = r
print('AEP minimum found at iteration number %f'%aep_min_iter)
print('AEP maximum found at iteration number %f'%aep_max_iter)